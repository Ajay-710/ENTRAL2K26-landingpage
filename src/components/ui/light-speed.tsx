...
export function LightSpeed({
    ...
    const uniformsRef = useRef<{ time?: WebGLUniformLocation | null; resolution?: WebGLUniformLocation | null }>({});
    ...
    useEffect(() => {
        ...
        if (!canvas) return; // Should not happen if ref matches but let's be safe
        ...
        try {
            ...
            if (fragmentSource !== DEFAULT_FRAG) {
                try {
                    // Re-compile VS because it might be consumed? No, but let's be safe.
                    // Actually VS is fine if compile succeeded.
                    // But 'compile' function creates new shader object.
                    // Let's just re-do it cleanly.
        ...
        if (!prog) return; // Should be handled by catch/return above
        ...
        // render loop
        // Using simple time offset
        const startTimeOffset = performance.now();
        ...
        const loop = (t: number) => {
            ...
            const now = (t - startTimeOffset) * 0.001 * (speed || 1);
        ...
        return () => {
            ...
            if (gl && programRef.current) {
                ...
                // WebGL: typically detach and delete
                // We'll roughly follow the snippet logic
                gl.useProgram(null);
                gl.deleteProgram(p);
            }
